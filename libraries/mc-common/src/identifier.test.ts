...
describe("Identifier validation", () => {
  ...
  it("validate is identifier", () => {
    ...
    const resultC = Identifier.isId(new Identifier("test", "path"));
    const resultD = Identifier.isId(BLOCK);
    const resultE = Identifier.isId(BLOCK?.permutation);
    const resultF = Identifier.isId(ITEM);
    const resultG = Identifier.isId(ENTITY);
    const resultH = Identifier.isId(DIM.getBiome(VECTOR3_ZERO));
    const resultI = Identifier.isId(BLOCK.type);
    const resultJ = Identifier.isId(ENTITY);
    const resultK = Identifier.isId(ITEM.type);
    const resultL = Identifier.isId(EFFECT);
    const resultM = Identifier.isId(ENCHANT);
  ...
  it("accepts a valid identifier", () => {
    ...
    expect(resultJ).toEqual(new Identifier("minecraft", "creeper"));
